...
| Attacker floods or injects strings into the audit log via the CSP report endpoint | `POST /api/csp-report` | Behind `require_auth` (the same-origin `report-uri` POST carries the session cookie), so it is not an unauthenticated write path. Body capped at 8 KB; only a fixed set of report fields is recorded, each truncated to 512 chars, as `dashboard.csp_violation`. A violation while the operator views attacker data is itself the signal - a candidate output-encoding gap. |
