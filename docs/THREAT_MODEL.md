...
| Attacker injects markup into the operator's authenticated dashboard via session data rendered into `innerHTML` (typed commands, bridge responses, captured usernames/passwords) | Dashboard SPA (`app.js`) | (1) `escapeText` HTML-escapes `& < > " '` at every interpolation; (2) `SecurityHeadersMiddleware` sets `Content-Security-Policy: script-src 'self'` (no `'unsafe-inline'`, and `style-src 'self'` likewise) as the backstop so an injected script cannot execute even if a value reaches the DOM unescaped, plus `X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `frame-ancestors 'none'`, `Referrer-Policy: no-referrer`. The CSP `report-uri` points at the tripwire below. |
| Attacker floods or injects strings into the audit log via the CSP report endpoint | `POST /api/csp-report` | Behind `require_auth` (the same-origin `report-uri` POST carries the session cookie), so it is not an unauthenticated write path. Body capped at 8 KB; only a fixed set of report fields is recorded, each truncated to 512 chars, as `dashboard.csp_violation`. A violation while the operator views attacker data is itself the signal - a candidate output-encoding gap. **CSRF (audit L7, accepted residual):** this state-changing POST deliberately omits the `require_csrf` token the other mutations require, because the browser sends CSP reports automatically and cannot attach the `X-Anglerfish-CSRF` header. `SameSite=Strict` blocks a cross-site page from sending the session cookie, and the worst a forged same-origin POST achieves is one capped, fixed-field tripwire row the operator can see. |
...
| **Unicode evasion** - a zero-width, compatibility-form, or combining character spliced into a flagged token (`I am an A‍I`, fullwidth `ＡＩ`, `ig̀nore`) to slip past the ASCII-literal patterns (audit H2) | Both scan paths (`InjectionScorer.score`, `OutputFilter.check`) canonicalise via `normalise_for_scan`: NFKD folds compatibility forms onto ASCII and decomposes precomposed accents, combining marks (`Mn`/`Mc`/`Me`) and `Cf` format characters (zero-width, BOM, soft-hyphen, bidi) are stripped, then the regex runs. Corpus carries the zero-width and combining-mark bypass cases. The original attacker bytes are preserved in the prompt and capture. | **Cross-script confusable homoglyphs** (Cyrillic `а` vs Latin `a`) are a different script with no decomposition to Latin, so they still evade a literal pattern. Low impact: an attacker crafting homoglyph evasion has almost certainly already concluded it is a honeypot. A confusables-folding step is the follow-on if it becomes material. |
...
| **Attacker spoofs the audited ``callback_source_ip`` via ``X-Forwarded-For``** (audit M4) | The receiver records the connection peer (``request.client``), never the raw header. The attacker triggering the callback controls ``X-Forwarded-For``, and a standards-compliant proxy *appends* the real peer to the right, so trusting it would let the attacker frame a third party or blank their own exfil node. Behind a proxy the operator sets uvicorn ``--proxy-headers`` + ``forwarded_allow_ips`` (default loopback) so the peer is the proxy-validated client. | If the operator widens ``forwarded_allow_ips`` to untrusted ranges, a client in those ranges can again influence the derived peer. That is a uvicorn-level trust configuration, documented, not a receiver code path. |
| **Honeytoken secret payloads leak through an export.** The dashboard exports the registry three ways (Stage 13). Payloads (the fake AWS credential block, the SSH private key) are tracking beacons; landing them in a shared intel feed tips off third parties and pollutes that feed. | The shareable formats (``GET /api/export/stix``, ``/api/export/misp``) and the SPA registry view (``GET /api/honeytokens``) emit identifiers and callback URLs only, never the payload. The full payload travels in exactly one path, ``GET /api/export/honeytoken_report`` - an auth-gated CSV download marked operator-only by its filename and module docs, never pushed to a third party. | An operator can still hand-share the honeytoken_report CSV. This is operator responsibility; the format split makes the safe path the default path, matching the STIX/MISP identifier-only default. |
...
5. **Audit writes `fsync` on the event loop (audit L8, accepted).** `AuditLog.record` does a synchronous `write` + `fsync` under a lock, and most callers are coroutines, so each write briefly stalls the event loop. Kept deliberately: per-event durability and strict append ordering are the log's contract (the `chattr +a` tamper-evidence story depends on records reaching disk in order, not batched or reordered by a writer thread). Audit volume is security events, not per-packet, so the stall is bounded and rare; offloading to a thread or queue would trade that guarantee for throughput the honeypot does not need.
